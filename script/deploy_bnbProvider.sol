...
contract MoolahVaultDeploy is Script {
  ...
  function run() public {
    ...
    console.log("Lista WBNB Vault BNBProvider implementation: ", address(impl));

    // Deploy Lista WBNB Vault BNBProvider proxy
    ERC1967Proxy proxy = new ERC1967Proxy(
      address(impl),
      abi.encodeWithSelector(impl.initialize.selector, deployer, deployer)
    );
    console.log("Lista WBNB Vault BNBProvider proxy: ", address(proxy));

    // Deploy MEV WBNB Vault BNBProvider proxy
    BNBProvider impl2 = new BNBProvider(moolah, mevVault, asset);
    console.log("MEV WBNB Vault BNBProvider implementation: ", address(impl2));
    ERC1967Proxy proxy2 = new ERC1967Proxy(
      address(impl2),
      abi.encodeWithSelector(impl2.initialize.selector, deployer, deployer)
    );
    console.log("MEV WBNB Vault BNBProvider proxy: ", address(proxy2));
