...
/**
 * @title WbETHV3Provider
 * @author Lista DAO
 * @notice wbETH/WETH V3 LP vault (Ethereum) — identical to {WstETHV3Provider}, paired with a
 *         {WbETHV3DexAdapter}. A lean {V3Provider}: no reward-token mirroring and no per-user deposit
 *         tracking — it inherits the base deposit / withdraw / redeem flow unchanged and only adds the
 *         BOT-gated rebalance forwarder.
 */
contract WbETHV3Provider is V3Provider {
