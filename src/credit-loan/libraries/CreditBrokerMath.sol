...
library CreditBrokerMath {
   ...
   * @return interestPerPosition Outstanding interest per position, index-aligned with `positions`
   * @return penaltyPerPosition Delay penalty per position, index-aligned with `positions`
   */
  ...
  {
    interestPerPosition = new uint256[](positions.length);
    penaltyPerPosition = new uint256[](positions.length);
    ...
    for (uint256 i = 0; i < positions.length; i++) {
      ...
      uint256 penalty = getPenaltyForCreditPosition(remainingPrincipal, remainingInterest, p.end, graceConfig);
      interestPerPosition[i] = remainingInterest;
      penaltyPerPosition[i] = penalty;
      interestAndPenalty += remainingInterest + penalty;
