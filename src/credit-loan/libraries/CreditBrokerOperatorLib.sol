...
library CreditBrokerOperatorLib {
  ...
  function repayAll(
    ...
    (
      uint256 interestAndPenalty,
      uint256 badDebtPrincipal,
      uint256 debtAtMoolah,
      uint256 totalDebt,
      uint256[] memory interestPerPosition,
      uint256[] memory penaltyPerPosition
    ) = CreditBrokerMath.previewRepayAllAmounts(
        onBehalf,
        positions,
        ctx.graceConfig,
        address(ctx.moolah),
    ...
    // emit the same per-position breakdown `repay` does, so an indexer reconstructing interest
    // and penalty revenue sees every position regardless of which path closed it, then wipe
    for (uint256 i = 0; i < positions.length; i++) {
      ...
      emit RepaidFixedLoanPosition(
        ...
        p.end,
        p.apr,
        p.principal, // every position is cleared in full
        p.principal - p.principalRepaid,
        interestPerPosition[i],
        penaltyPerPosition[i],
        p.interestRepaid + interestPerPosition[i],
        p.isBadDebt
      );
  ...
  ///      would let one stranded position block partial repay of, or a new borrow against,
  ///      every other position on the account.
  ///      This is the broker-level floor only. Moolah keeps its own account-level floor in
  ///      `_checkBorrowAssets`, so fully repaying a healthy position can still revert with
  ///      `remain borrow too low` while a stranded position keeps the account's aggregate
  ///      Moolah debt inside `(0, minLoan)`. Clearing the stranded leg, or `repayAll`, is the
  ///      way out of that case.
