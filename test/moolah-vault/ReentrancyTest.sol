...
import { ERC1820Registry } from "moolah-vault/mocks/ERC1820Registry.sol";
import { ERC777Mock, IERC1820Registry } from "moolah-vault/mocks/ERC777Mock.sol";
...
contract ReentrancyTest is IntegrationTest, IERC1820Implementer {
  ...
  function setUp() public override {
    ...
    vault = newMoolahVault(OWNER, OWNER, address(moolah), address(reentrantToken), "Moolah Vault", "MMV");
    ...
    reentrantToken.setBalance(SUPPLIER, 100_000 ether); // SUPPLIER supplies 100_000e18 tokens to MoolahVault.
    ...
    console2.log(
      "Supplier deposited %s loanTokens to moolah-vault_no_timelock in exchange for %s shares.",
  ...
  function tokensToSend(address, address from, address to, uint256 amount, bytes calldata, bytes calldata) external {
    if ((from == attacker) && (amount == 5000)) {
      ...
      vm.startPrank(attacker);
      IMoolahVault(to).withdraw(1, attacker, attacker);
      vm.stopPrank();
    }
  }
