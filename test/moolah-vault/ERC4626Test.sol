...
contract ERC4626Test is IntegrationTest, IMoolahFlashLoanCallback {
  ...
  function testMint(uint256 assets) public {
    ...
    assertEq(moolah.expectedSupplyAssets(allMarkets[0], address(vault)), assets, "expectedSupplyAssets(vault)");
  }
  ...
  function testWithdrawAll(uint256 assets) public {
    ...
    assertEq(moolah.expectedSupplyAssets(allMarkets[0], address(vault)), 0, "expectedSupplyAssets(vault)");
  }
  ...
  function testWithdrawMoreThanBalanceAndLiquidity(uint256 deposited, uint256 assets) public {
    ...
    moolah.supplyCollateral(allMarkets[0], type(uint128).max, BORROWER, hex"");
    moolah.borrow(allMarkets[0], 1, 0, BORROWER, BORROWER);
  ...
  function testMaxWithdraw(uint256 depositedAssets, uint256 borrowedAssets) public {
    ...
    vm.startPrank(BORROWER);
    moolah.supplyCollateral(allMarkets[0], type(uint128).max, BORROWER, hex"");
    moolah.borrow(allMarkets[0], borrowedAssets, 0, BORROWER, BORROWER);
  ...
  function testMaxWithdrawFlashLoan(uint256 supplied, uint256 deposited) public {
    ...
    vm.prank(SUPPLIER);
    moolah.supply(allMarkets[0], supplied, 0, ONBEHALF, hex"");
    ...
    moolah.flashLoan(address(loanToken), loanToken.balanceOf(address(moolah)), hex"");
  }
  ...
  function testMaxDeposit() public {
    ...
    vm.prank(SUPPLIER);
    moolah.supply(allMarkets[0], 1 ether, 0, SUPPLIER, hex"");

    vm.startPrank(BORROWER);
    moolah.supplyCollateral(allMarkets[0], 2 ether, BORROWER, hex"");
    moolah.borrow(allMarkets[0], 1 ether, 0, BORROWER, BORROWER);
    vm.stopPrank();
  ...
  function onMoolahFlashLoan(uint256, bytes memory) external view {
