...
contract FeeTest is IntegrationTest {
  ...
  function setUp() public override {
    ...
    for (uint256 i; i < NB_MARKETS; ++i) {
      ...
      vm.prank(SUPPLIER);
      moolah.supply(marketParams, MAX_TEST_ASSETS, 0, ONBEHALF, hex"");

      uint256 collateral = uint256(MAX_TEST_ASSETS).wDivUp(marketParams.lltv);
      collateralToken.setBalance(BORROWER, collateral);

      vm.startPrank(BORROWER);
      moolah.supplyCollateral(marketParams, collateral, BORROWER, hex"");
      moolah.borrow(marketParams, MAX_TEST_ASSETS, 0, BORROWER, BORROWER);
      vm.stopPrank();
    }
  ...
  function testAccrueFeeWithinABlock(uint256 deposited, uint256 withdrawn) public {
    ...
    // The deposited amount is rounded down on Moolah and thus cannot be withdrawn in a block in most cases.
