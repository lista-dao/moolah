...
contract BaseInvariantTest is InvariantTest {
  ...
  function setUp() public virtual override {
    ...
    for (uint256 i = 2; i <= 6; ++i) {
      ...
      vm.startPrank(OWNER);
      moolah.enableLltv(_marketParams.lltv);
      moolah.createMarket(_marketParams);
      vm.stopPrank();
  ...
  modifier authorized(address onBehalf) {
    if (onBehalf != msg.sender && !moolah.isAuthorized(onBehalf, msg.sender)) {
      vm.prank(onBehalf);
      moolah.setAuthorization(msg.sender, true);
    }

    _;

    if (moolah.isAuthorized(onBehalf, msg.sender)) {
      vm.prank(onBehalf);
      moolah.setAuthorization(msg.sender, false);
    }
  }
  ...
  function _supplyAssets(
    MarketParams memory _marketParams,
    uint256 assets,
    address onBehalf
  ) internal logCall("supplyAssets") {
    loanToken.setBalance(msg.sender, assets);

    vm.prank(msg.sender);
    moolah.supply(_marketParams, assets, 0, onBehalf, hex"");
  }
  ...
  function _supplyShares(
    ...
    uint256 shares,
    address onBehalf
  ) internal logCall("supplyShares") {
    (uint256 totalSupplyAssets, uint256 totalSupplyShares, , ) = moolah.expectedMarketBalances(_marketParams);

    loanToken.setBalance(msg.sender, shares.toAssetsUp(totalSupplyAssets, totalSupplyShares));

    vm.prank(msg.sender);
    moolah.supply(_marketParams, 0, shares, onBehalf, hex"");
  }
  ...
  function _withdraw(
    ...
    vm.prank(msg.sender);
    moolah.withdraw(_marketParams, assets, shares, onBehalf, receiver);
  }
  ...
  function _borrow(
    ...
    vm.prank(msg.sender);
    moolah.borrow(_marketParams, assets, shares, onBehalf, receiver);
  }
  ...
  function _repayAssets(
    ...
    uint256 assets,
    address onBehalf
  ) internal logCall("repayAssets") {
    loanToken.setBalance(msg.sender, assets);

    vm.prank(msg.sender);
    moolah.repay(_marketParams, assets, 0, onBehalf, hex"");
  }
  ...
  function _repayShares(
    ...
    vm.prank(msg.sender);
    moolah.repay(_marketParams, 0, shares, onBehalf, hex"");
  }
  ...
  function _supplyCollateral(
    ...
    uint256 assets,
    address onBehalf
  ) internal logCall("supplyCollateral") {
    collateralToken.setBalance(msg.sender, assets);

    vm.prank(msg.sender);
    moolah.supplyCollateral(_marketParams, assets, onBehalf, hex"");
  }
  ...
  function _withdrawCollateral(
    ...
    address onBehalf,
    address receiver
  ) internal authorized(onBehalf) logCall("withdrawCollateral") {
    vm.prank(msg.sender);
    moolah.withdrawCollateral(_marketParams, assets, onBehalf, receiver);
  }

  function _liquidateSeizedAssets(
    MarketParams memory _marketParams,
    address borrower,
    ...
    vm.prank(msg.sender);
    moolah.liquidate(_marketParams, borrower, seizedAssets, 0, hex"");
  }

  function _liquidateRepaidShares(
    MarketParams memory _marketParams,
    address borrower,
    uint256 repaidShares
  ) internal logCall("liquidateRepaidShares") {
    (, , uint256 totalBorrowAssets, uint256 totalBorrowShares) = moolah.expectedMarketBalances(_marketParams);

    loanToken.setBalance(msg.sender, repaidShares.toAssetsUp(totalBorrowAssets, totalBorrowShares));

    vm.prank(msg.sender);
    moolah.liquidate(_marketParams, borrower, 0, repaidShares, hex"");
  }
  ...
  function setFeeNoRevert(uint256 marketSeed, uint256 newFee) external {
    ...
    if (newFee == moolah.market(_id).fee) return;

    vm.prank(OWNER);
    moolah.setFee(_marketParams, newFee);
  }
  ...
  function withdrawAssetsOnBehalfNoRevert(
    uint256 marketSeed,
    uint256 assets,
  ...
  function borrowAssetsOnBehalfNoRevert(
    uint256 marketSeed,
    uint256 assets,
    uint256 onBehalfSeed,
    address receiver
