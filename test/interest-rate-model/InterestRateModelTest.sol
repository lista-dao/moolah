...
contract InterestRateModelTest is Test {
  ...
  function setUp() public {
    ...
    selectors[0] = InterestRateModelTest.handleBorrowRate.selector;
  ...
  function testDeployment() public {
    ...
    new InterestRateModel(address(0));
  }
  ...
  function testFirstBorrowRateUtilizationOne() public {
    ...
    assertEq(irm.borrowRate(marketParams, market), uint256(ConstantsLib.INITIAL_RATE_AT_TARGET * 4), "avgBorrowRate");
    assertEq(irm.rateAtTarget(marketParams.id()), ConstantsLib.INITIAL_RATE_AT_TARGET, "rateAtTarget");
  }
  ...
  function testRateAfter45DaysUtilizationAboveTargetPingEveryMinute() public {
    ...
    assertApproxEqRel(
      ...
      initialBorrowAssets.wMulDown(1.457 ether),
      0.3 ether,
      "totalBorrowAssets"
    );
  }
  ...
  function testFirstBorrowRateView(Market memory market) public view {
    vm.assume(market.totalBorrowAssets > 0);
    vm.assume(market.totalSupplyAssets >= market.totalBorrowAssets);
  ...
  function _curve(int256 rateAtTarget, int256 err) internal pure returns (uint256) {
    ...
    if (err < 0) {
      return
        uint256(((WAD - WAD.wDivToZero(ConstantsLib.CURVE_STEEPNESS)).wMulToZero(err) + WAD).wMulToZero(rateAtTarget));
    } else {
